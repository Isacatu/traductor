...
{
    if (!nodo)
        return new NodoAVL(w);
    if (w < nodo->palabra)
        nodo->iz = insertarAVL(nodo->iz, w);
    else if (w > nodo->palabra)
        nodo->de = insertarAVL(nodo->de, w);
    else
    ...
    int b = balance(nodo);
    if (b > 1 && w < nodo->iz->palabra)
        return rotR(nodo);
    if (b < -1 && w > nodo->de->palabra)
        return rotL(nodo);
    if (b > 1 && w > nodo->iz->palabra)
    {
        nodo->iz = rotL(nodo->iz);
        return rotR(nodo);
    }
    if (b < -1 && w < nodo->de->palabra)
    {
        nodo->de = rotR(nodo->de);
        return rotL(nodo);
    }
...
void export_to_csv(pqxx::connection &C)
{
    ...
    if (!o)
    {
        cerr << "No puedo abrir el archivo " << fn << "\n";
        return;
    }
    if (!exists)
        o << "User,Source Language,Source Word,Target Language,Target Word\n";
...
int main()
{
    ...
    string connStr = "dbname=" + string(dbName) + " user=" + dbUser + " password=" + dbPassword + " host=" + dbHost + " port=" + dbPort;
    ...
    originalFile.close();
    encriptadoFile.close();

    // Definición de la función lambda 'compressFile' que toma dos parámetros:
    // - inputFile: archivo que será comprimido.
    // - outputFile: archivo donde se guardarán los datos comprimidos.
    auto compressFile = [](const string& inputFile, const string& outputFile) {
        // Abrir el archivo de entrada en modo binario
        ifstream in(inputFile, ios::binary);
        if (!in) {
            cerr << "No se pudo abrir el archivo de entrada: " << inputFile << "\n";
            return;
        }
        ...
        // Calcular el tamaño máximo para la compresión
        size_t bound = ZSTD_compressBound(inputData.size());
        vector<char> compressedData(bound);
    
        // Comprimir los datos
        /*
            -compressedData.data(): el puntero al espacio donde se almacenarán los datos comprimidos.
            -bound: el tamaño maximo del espacio de destino.
            -inputData.data(): el puntero a los datos que se van a comprimir.
            -inputSize: el tamaño de los datos que se van a comprimir.
            -ZSTD_CLEVEL_DEFAULT: el nivel de compresión.
        */
        size_t compressedSize = ZSTD_compress(compressedData.data(), bound, inputData.data(), inputData.size(), ZSTD_CLEVEL_DEFAULT);
    
        if (ZSTD_isError(compressedSize)) {
            cerr << "Error al comprimir: " << ZSTD_getErrorName(compressedSize) << "\n";
            return;
        }
        ...
        cout << "Archivo comprimido: " << outputFile << " (" << compressedSize << " bytes)\n";
    };
    

    compressFile("original.txt", "original.txt.zst");
    compressFile("encriptado.txt", "encriptado.txt.zst");
